...
export default function Contractors() {
  ...
  return (
    <div>
      <PageHeader
        title="Contractors"
        subtitle={`${table.total} contractors engaged for execution work`}
        actions={allowCreate ? <Button icon={Plus} onClick={openCreate}>New Contractor</Button> : null}
      />
      <PageBody>
        <Card padded={false}>
          <DataTable
            ...
            emptyState={{ title: 'No contractors found', description: 'Add your first contractor to get started.', action: allowCreate ? { label: 'New Contractor', icon: Plus, onClick: openCreate } : undefined }}
            bulkActions={allowDelete ? [{ label: 'Delete', icon: Trash2, onClick: async (ids) => { await Promise.all(ids.map((id) => vendorApi.remove(id))); toast.success(`${ids.length} contractor(s) deleted`); setSelected([]); table.refresh() } }] : []}
            toolbar={
              <>
                <SearchInput value={table.query} onChange={table.setQuery} placeholder="Search contractors…" className="w-full max-w-xs" />
      ...
      <Drawer
        ...
        title={drawer.mode === 'create' ? 'New Contractor' : 'Edit Contractor'}
        footer={
          <>
            <Button variant="secondary" onClick={() => setDrawer({ open: false, mode: 'create', record: null })}>Cancel</Button>
            <Button loading={saving} onClick={handleSubmit(onSubmit)}>{drawer.mode === 'create' ? 'Create Contractor' : 'Save changes'}</Button>
          </>
        }
      >
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <Field label="Contractor Name" required error={errors.name?.message}>
