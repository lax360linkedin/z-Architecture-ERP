...
export default function Vendors() {
  ...
  const table = useDataTable(vendorApi.list, { pageSize: 8, initialFilters: { category: 'all', status: 'all' } })
  ...
  function openCreate() {
    reset({ category: vendorCategories[0], status: 'active', rating: 4, since: new Date().toISOString().slice(0, 10) })
  ...
  async function onSubmit(values) {
    setSaving(true)
    try {
      const payload = { ...values, rating: Number(values.rating) || 0 }
  ...
  const columns = [
    ...
    { key: 'category', header: 'Category', render: (v) => <Badge>{v.category}</Badge> },
  ...
  return (
    <div>
      <PageHeader
        title="Vendors"
        subtitle={`${table.total} vendors and suppliers`}
        actions={allowCreate ? <Button icon={Plus} onClick={openCreate}>New Vendor</Button> : null}
      />
      <PageBody>
        <Card padded={false}>
          <DataTable
            ...
            emptyState={{ title: 'No vendors found', description: 'Add your first vendor to start building your supply base.', action: allowCreate ? { label: 'New Vendor', icon: Plus, onClick: openCreate } : undefined }}
            bulkActions={allowDelete ? [{ label: 'Delete', icon: Trash2, onClick: async (ids) => { await Promise.all(ids.map((id) => vendorApi.remove(id))); toast.success(`${ids.length} vendor(s) deleted`); setSelected([]); table.refresh() } }] : []}
            toolbar={
              <>
                <SearchInput value={table.query} onChange={table.setQuery} placeholder="Search vendors…" className="w-full max-w-xs" />
      ...
      <Drawer
        ...
        title={drawer.mode === 'create' ? 'New Vendor' : 'Edit Vendor'}
        footer={
          <>
            <Button variant="secondary" onClick={() => setDrawer({ open: false, mode: 'create', record: null })}>Cancel</Button>
            <Button loading={saving} onClick={handleSubmit(onSubmit)}>{drawer.mode === 'create' ? 'Create Vendor' : 'Save changes'}</Button>
          </>
        }
      >
