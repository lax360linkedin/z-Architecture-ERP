...
export default function FleetMaintenance() {
  ...
  const table = useDataTable(fleetApi.maintenance.list, { pageSize: 8, initialFilters: { status: 'all' } })
  ...
  function openCreate() {
    reset({ vehicle: vehicles[0]?.id || '', type: '', date: '', cost: '', status: 'in-progress' })
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      if (drawer.mode === 'create') {
        await fleetApi.maintenance.create(payload)
        toast.success('Maintenance record added')
      } else {
        await fleetApi.maintenance.update(drawer.record.id, payload)
  ...
  async function confirmDelete() {
    setSaving(true)
    try {
      await fleetApi.maintenance.remove(confirm.record.id)
  ...
  async function markCompleted(record) {
    await fleetApi.maintenance.update(record.id, { status: 'completed' })
    toast.success(`${record.type} marked as completed`)
    table.refresh()
  }

  const columns = [
    { key: 'vehicle', header: 'Vehicle', render: (r) => vehicles.find((v) => v.id === r.vehicle)?.regNo || r.vehicle },
      ...
      <PageBody>
        <Card padded={false}>
          <DataTable
            ...
            bulkActions={allowDelete ? [{ label: 'Delete', icon: Trash2, onClick: async (ids) => { await Promise.all(ids.map((id) => fleetApi.maintenance.remove(id))); toast.success(`${ids.length} record(s) deleted`); setSelected([]); table.refresh() } }] : []}
            toolbar={
              <>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <Field label="Type" required error={errors.type?.message}>
            <Input placeholder="e.g. Engine Overhaul" {...register('type', { required: 'Type is required' })} />
          </Field>
